...
review of the existing frameworks supporting \gls{metrology}, i.e., the
\ac{VIM} \cite{JCGMVIM}, the \ac{GUM} \cite{JCGMGUM}, and the \ac{QA4EO}.
Section~\ref{sec:level1} and section~\ref{sec:level2} provide the guidelines to
uncertainty estimation for SAR level-1 and level-2 data products, respectively,
as developed in the \ac{ESA} funded \ac{CANVAS} project. Finally,
section~\ref{sec:uncertainty_propagation} describes the methods for propagating
...
% Approach
In order to establish a systematic process for generating uncertainty budgets,
and ensuring that \ac{EO} data are interoperable, temporally stable, and fit
for scientific, operational and decision-making purposes, the \ac{QA4EO} and
\ac{EO} projects acknowledged in \cite{QA4EO3} have adapted the stages,
suggested in \ac{GUM}, required to support the development of a measurement
model and from that an uncertainty analysis. These \ac{EO} specific stages of
uncertainty analysis are given by \cite{QA4EO3} as follows:

\begin{itemize}
  \item Step 1: Define the \gls{measurand} and measurement model.
  \item Step 2: Establish the \gls{traceability} with a diagram.
  \item Step 3: Evaluate each source of uncertainty and fill out an effects table.
  \item Step 4: Calculate the data product and uncertainties.
  \item Step 5: Record information about the \gls{uncertainty analysis} for long term data preservation purposes and summarise for today's users.
\end{itemize}

These steps are discussed in detail in \cite{QA4EO3}. In
sections~\ref{sec:level1} and \ref{sec:level2}, the application to SAR
level-1 and level-2 data is discussed.
