...
uncertainty analysis should be organized around the calibrated \ac{SLC}, the
...
\ac{SLC} product inherits uncertainty contributions from radiometric gain and
...
\ac{SLC} products, their co-registration, the baseline knowledge, and the
statistics of the interferometric estimator. An \ac{NRB}/\ac{ARD} product inherits the uncertainty of the
source SAR acquisition and adds uncertainty from radiometric terrain correction, \ac{DEM} and ancillary
data, geocoding, resampling, masks, and map-grid definition.

The following product-based structure is adopted:
\begin{itemize}
  \item a general \ac{GUM}-oriented framework for Level-1 products;
  \item a calibrated single-channel \ac{SLC} product as the main Level-1
  example;
\item a polarimetric \ac{SLC} product, for which the scattering matrix is the
  principal measurand;
  \item an interferometric product, whose principal observables are coherence,
  phase, and related quality indicators;
\item a geocoded \ac{ARD}/\ac{NRB} product, for which the measurand is terrain-normalised
...
\item auxiliary-data terms, such as orbit, attitude, \ac{DEM}, and surveyed reference-target uncertainties.
\end{itemize}


The uncertainty budget shall also include the \ac{Cal/Val} reference and the
...
Auxiliary data files are part of the measurement model. Orbit and attitude
products, \acp{DEM}, meteorological fields, ionospheric maps, antenna patterns,
calibration look-up tables, masks, and processor annotation files shall
therefore be assigned uncertainty terms, validity domains, and correlation
scales. Atmospheric propagation terms include atmospheric path delay and
attenuation; ionospheric terms include dispersive group delay, and Faraday
rotation. These terms can affect \ac{SLC}, and interferometric products and are not
limited to geocoded products.
...
A calibrated Level-1 \ac{SLC} is the natural reference example because it is the product from which most other Level-1 quality indicators are derived. Its measurand is the calibrated complex pixel value together with the associated radiometric and geometric interpretation.
...
measurement model of the Level-1 \ac{SLC} product.
...
observables. For a calibrated \ac{SLC}, the uncertainty budget is conveniently organized into three observable-oriented branches, each derived from the same calibrated complex sample but leading to a different product-level uncertainty statement.
...
or $\sigma^{0}$ is estimated from the \ac{SLC}, these terms dominate the radiometric quality statement.
...
\input{slc_traceability_diagram}
...
the geometric branch is dominant. The same \ac{SLC} product thus supports
...
For a calibrated \ac{SLC}, it is therefore recommended to report uncertainty in
...
coherency representation derived from it. All single-channel \ac{SLC} uncertainty effects still
...
For a polarimetric \ac{SLC}, the reported uncertainty should be associated with
...
For two calibrated \ac{SLC} products $s_{1}$ and $s_{2}$, the basic
...
\begin{itemize}
  \item uncertainties inherited from each input \ac{SLC};
...
uncertainty of the external \ac{DEM}, geoid or height reference, baseline, and orbit
...
estimation window. For many low-relief coherence applications this \ac{DEM}-driven
...
\begin{table*}[ht]
  ...
  \begin{tabular}{|p{3.0cm}|c|c|c|c|p{3.2cm}|}
    ...
    Input \ac{SLC} radiometric/phase errors & Yes & Yes & 2--3 & structured / image pair & inherited covariance term \\ \hline
...
\acro{NRB} is a Level-1 Analysis Ready Data product in which the calibrated SAR
measurement is terrain-corrected, radiometrically normalised, resampled, and
delivered on a defined map grid. The CEOS \ac{NRB} Product Family Specification
therefore extends the uncertainty problem beyond the source \ac{SLC} or detected
product: the final measurand is not only a calibrated backscatter value, but a
terrain-normalised backscatter value assigned to a specific projected pixel
with associated metadata, masks, and ancillary-data lineage \cite{NRB_PFS}.

\subsubsection{Step 1: Definition of measurand and measurement model}

For an \ac{NRB} pixel, a generic measurand can be written as
\begin{equation}
  y_{\rm NRB}(\mathbf{p}) =
  ...
  \label{eq:nrb_model}
\end{equation}
where $\mathbf{p}$ is the output map-grid pixel, $z_{\rm src}$ is the calibrated source-product backscatter or intensity quantity, $T_{\rm rad}$ is the radiometric terrain-normalisation and geocoding operator, $\theta_{\rm loc}$ is the local incidence-angle information, $\mathbf{d}_{\rm DEM}$ denotes \ac{DEM} and terrain variables, $\mathbf{m}$ denotes product masks and validity flags, and $\Delta y_{\rm NRB}$ represents residual modelling and implementation effects. The corresponding geometric measurand is the map location of the output pixel,
\begin{equation}
...
The \ac{NRB} uncertainty budget should be organized into inherited and product-generation terms:
\begin{itemize}
  ...
  \item \textbf{Radiometric terrain-normalisation terms:} \ac{DEM} height uncertainty, slope/aspect uncertainty, local incidence-angle estimation, scattering-area model, terrain-flattening convention, and residual layover or shadow contamination.
  \item \textbf{Geometric and map-grid terms:} coordinate reference system definition, pixel coordinate convention, grid origin, pixel spacing, orthorectification model, \ac{DEM}-induced geolocation error, and reference-image or GCP uncertainty used for validation.
...
\ac{ARD}/\ac{NRB} Level-1 product. The diagram separates the inherited source-product
...
is important because an \ac{NRB} pixel is not only a calibrated backscatter value;
...
\begin{figure}[H]
  ...
  \caption{Traceability diagram for a geocoded \ac{ARD}/\ac{NRB} Level-1 product. The final measurands are the terrain-normalised backscatter value $y_{\rm NRB}(\mathbf{p})$ and its associated map-grid location $\mathbf{g}_{\rm NRB}$.}
...
\ac{NRB}/\ac{ARD} products. The table distinguishes radiometric uncertainty, geometric
...
\begin{table*}[ht]
  \scriptsize
  \centering
  \caption{Uncertainty effects table for the geocoded \ac{NRB} Level-1 product.}
  \resizebox{\textwidth}{!}{%
  \begin{tabular}{|p{3.2cm}|c|c|c|c|p{3.3cm}|p{3.2cm}|}
    ...
    \ac{DEM} height, slope, and aspect & Yes & Yes & weak & 2--3 & spatially correlated / terrain tile & local-incidence and geolocation covariance \\ \hline
...
For \ac{NRB}, uncertainty propagation should account for both radiometric and geometric covariance. In compact form,
\begin{equation}
...
\end{equation}
where $\mathbf{U}_{\rm src}$ contains inherited source-product covariance, $\mathbf{U}_{\rm aux}$ contains \ac{DEM}, orbit, incidence-angle, and other ancillary-data covariance, $\mathbf{U}_{\rm proc}$ contains geocoding and resampling implementation terms, and $u_{\rm mask}$ represents uncertainty associated with validity classification. These terms are often spatially correlated: for example, a \ac{DEM} bias or orbit error can affect many neighbouring pixels coherently, whereas speckle and feature-identification errors may be more local. A \ac{GUM}-compliant \ac{NRB} uncertainty statement should therefore avoid treating all terms as independent per-pixel random noise.

\subsubsection{Step 6: Product-level reporting}

For CANVAS, the uncertainty statement for \ac{NRB}/\ac{ARD} products should report at least:
\begin{itemize}
  \item inherited source-product radiometric and geometric uncertainty;
  \item uncertainty associated with \ac{DEM}, local incidence angle, terrain correction, and terrain-normalisation convention;
  ...
  \item the traceability of ancillary data and processing versions used to generate the \ac{NRB} product.
\end{itemize}

This subsection complements the \ac{SLC}-focused uncertainty analysis above. It makes explicit that \ac{NRB}/\ac{ARD} procedures inherit source-acquisition uncertainty but also require a separate product-generation uncertainty budget for terrain correction, geocoding, masking, and metadata lineage.
