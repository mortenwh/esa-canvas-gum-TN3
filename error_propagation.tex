...
\subsection{Example: \ac{SAR} Significant Wave Height Retrieval}
\label{sec: SWH}
...
\subsection{Example: \ac{SAR} Wind Retrieval}
\label{sec: OSWV}
...
\begin{equation}
	\mathrm{pdf}_{\mathbf{v}|\sigma_{0},f_{dc},\lambda_{C},\mathbf{v_{B}}} \propto \mathrm{pdf}_{\mathbf{v}|\mathbf{v_{B}}} \mathrm{pdf}_{\sigma_{0}|\mathbf{v}} \mathrm{pdf}_{\lambda_{C}|U} \mathrm{pdf}_{f_{w}|\mathbf{v},f_{dc}}
	\label{eq: AP pdf}
\end{equation}
...
known the wind speed ($U$), and $\mathrm{pdf}_{f_{w}|\mathbf{v}}$ is the
conditional \ac{PDF} of the prediction model of the \ac{DCA} wind component ($f_{w}$), known \ac{DC} ($f_{dc}$). $\mathbf{v_{B}}$ is
...
\noindent with $U$ the wind speed, $\phi$
the relative wind direction with respect to the antenna looking angle, and
$A_{i}, \ \forall i \in \left\{ 0,1,2 \right\}$, the fitting coefficients
derived empirically. The dependency of the coefficients $A_{i}$ on $\nu$ and $pol$ is implicit in this notation.
...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		\node [root_sub, below right=3cm and -6cm of CORE] (J_DOPP) {$J_{f_{w}} = \left( \frac{f_{w}^{M} - f_{w}^{S}}{\Delta f_{w}} \right)^2 + 0$}
		[sibling distance=5cm, level distance=1.5cm]
		child { node [node_black] {$\frac{\partial J_{f_{w}}}{\partial f_{w}^{M}}$}
			child { node [node_model_green, font=\tiny] {$f_{w}^{M} = f_{dc} - f_{elec} - f_{sca} - f_{att} - f_{ss} - f_{osc}+ 0$}
				[sibling distance=1.6cm, level distance=2.5cm]
				child { node [node_black, font=\tiny] {$\frac{\partial f_w^M}{\partial f_{elec}}$} 
					[level distance=1.5cm]
					child { node [leaf, draw=green!50!black] (U_ELE) {$u(f_{elec})$} } }
				child { node [node_black, font=\tiny] {$\frac{\partial f_w^M}{\partial f_{att}}$} 
					[level distance=1.5cm]
					child { node [leaf, draw=green!50!black] (U_ATT) {$u(f_{att})$} 
						node [effect, below left =0.3cm and -0.65cm of U_ATT] (EFF_UATT) {Orbital Fitting, \\ Antenna mis-pointing}
					} }
				child { node [node_black, font=\tiny] {$\frac{\partial f_w^M}{\partial f_{ss}}$}
					[level distance=1.5cm] 
					child { node [leaf, draw=green!50!black] (U_SS) {$u(f_{ss})$} } }
					child { node [node_black, font=\tiny] {$\frac{\partial f_w^M}{\partial f_{osc}}$}
					[level distance=1.5cm] 
					child { node [leaf, draw=green!50!black] (U_OSC) {$u(f_{osc})$} 
						node [effect, below left =0.3cm and -0.65cm of U_OSC] (EFF_UOSC) {Sea state and \\ current modelling}
					} }
				child { node [node_black, font=\tiny] (PAR_FDCM) {$\frac{\partial f_w^M}{\partial f_{dc}}$} 
					[level distance=1.5cm] }
				edge from parent [draw=green!50!black]
			}
			edge from parent [draw=green!50!black]
		}
		child { node [node_black] {$\frac{\partial J_{f_{w}}}{\partial f_{w}^{S}}$}
			[sibling distance=5cm, level distance=2.5cm]
			child { node [node_model_green] {$f_{w}^{S} = GMF_{f_{w}}(\theta,f_{dc},pol,\nu) + 0$}
				[sibling distance=1.6cm, level distance=1.5cm]
				child { node [node_black] {$\frac{\partial f_{w}^{S}}{\partial f_{dc}}$} child { node [leaf, draw=green!50!black] (U_FDC) {$u(f_{dc})$}
						node [effect, below=0.3cm of U_FDC] (EFF_FDC) {Scene variability, \\ Scene size, \\ Speckle, \\ Scalloping, \\ SNR} 
				} }
				child { node [node_black] {$\frac{\partial f_{w}^{S}}{\partial \theta}$} child { node [leaf, draw=green!50!black] (U_TH2) {$u(\theta)$} 
...
\lambda_{C}^{S}$ and $\Delta f_{w}=\Delta f_{w}^{M}+\Delta f_{w}^{S}$, which
represent the uncertainties of $\lambda_{C}$ and $f_{w}$, respectively.
...
\begin{itemize}
	\item $u(f_{dc})$: uncertainty of \ac{DC};
	\item $u(f_{elec})$: uncertainty due to antenna electronic mis-pointing;
	\item $u(f_{sca})$: uncertainty due to scalloping;
	\item $u(f_{att})$: uncertainty due to satellite attitude variations;
	\item $u(f_{osc})$: uncertainty of the estimate of the DCA ocean surface current component;
	\item $u(f_{ss})$: uncertainty of the estimate of the DCA sea state component;
\end{itemize}
...
\begin{table}[h]
		...
		\scriptsize % Font ridotto per far entrare i 15cm
		\setlength{\tabcolsep}{2pt} % Riduce lo spazio tra le colonne
		\caption{Effects Table for $J_{f_{w}}$}
		...
		\begin{tabularx}{\textwidth}{l | X | X | X | X}
			...
			\midrule
			\textbf{Affected Term} & $f_{dc}$ & $f_{p} \ (f_{att}$,$f_{elec})$ & $f_{ss}$,$f_{osc}$ & $f_{w}^{S}$ \\
			\midrule
			\textbf{Maturity} &  &  &  &  \\
			...
			\quad -- Magnitude & & & & \\
			\midrule
			\textbf{Sensitivity coefficient} & $\frac{\partial J_{f_{w}}}{\partial f_{w}^{M}} \frac{\partial f_{w}^{M}}{\partial f_{dc}}$ & $\frac{\partial J_{f_{w}}}{\partial f_{w}^{M}} \frac{\partial f_{w}^{M}}{\partial f_{elec,att}}$ & $\frac{\partial J_{f_{w}}}{\partial f_{w}^{M}} \frac{\partial f_{w}^{M}}{\partial f_{ss,osc}}$ & $\frac{\partial J_{f_{w}}}{\partial f_{w}^{S}}$ \\
			\bottomrule
		\end{tabularx}
